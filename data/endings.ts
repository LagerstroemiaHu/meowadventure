...
import { Skull, Crown, Star, Home, Heart, Zap, Ghost, Trash2, Camera, Brain, Shield, Users, HeartCrack, Baby, Smile, Frown, Swords, Flag, Eye, Lock, Bird, EggOff, Cat, Footprints, Scissors } from 'lucide-react';
...
export const ENDING_REGISTRY: Record<EndingType, EndingConfig> = {
    ...
    'STRAY_STARVED': {
        ...
        icon: Skull, color: 'text-stone-500', isGood: false, image: getImg('雪地中的瘦猫', '57534e')
    },
    'STRAY_FROZEN': {
        id: 'STRAY_FROZEN', title: '冻毙荒野', description: '生命值耗尽。在某个寒冷的冬夜，你蜷缩成一团，再也没有睁开眼。',
        icon: Ghost, color: 'text-cyan-400', isGood: false, image: getImg('寒风中的僵硬猫咪', '0ea5e9')
    },
    'STRAY_DOMESTICATED': {
        id: 'STRAY_DOMESTICATED', title: '被收编的灵魂', description: '哈气值归零。你放弃了流浪的尊严，在第一阶段就主动找了张饭票。虽然活着，但野性已死。',
        icon: Home, color: 'text-orange-400', isGood: false, image: getImg('窗台上的家猫背影', 'fbbf24')
    },

    // === STAGE 2: LORD ===
    'LORD_DEPOSED': {
        id: 'LORD_DEPOSED', title: '失去王座', description: '饱腹感归零。曾经的王因为太久没抢到食物，被年轻的挑战者赶下了台，饿死在领地边缘。',
        icon: Crown, color: 'text-stone-400', isGood: false, image: getImg('破碎的王冠与背影', '44403c')
    },
    'LORD_KILLED': {
        id: 'LORD_KILLED', title: '帮派喋血', description: '生命值归零。王座之下皆是枯骨。你在最后一次地盘争夺战中力竭而亡，死得像个斗士。',
        icon: Swords, color: 'text-red-600', isGood: false, image: getImg('巷战中倒下的伤猫', '991b1b')
    },
    'LORD_SOFT': {
    ...
    'MANSION_THROWN_OUT': {
        ...
        icon: Trash2, color: 'text-stone-600', isGood: false, image: getImg('豪宅大门紧闭猫在雨中', '57534e')
    },
    'MANSION_OBESITY': {
    ...
    'ACH_PHILO_UTOPIA': {
        ...
        icon: Flag, color: 'text-red-500', isGood: true, image: getImg('猫群聚集在红旗下', 'ef4444')
    },
    'ACH_PHILO_NIHILISM': {
    ...
    'ACH_RETURN_WILD': {
        ...
        icon: Bird, color: 'text-cyan-500', isGood: true, image: getImg('跳出窗户飞向蓝天', '06b6d4')
    },
    'ACH_CAGE_CAT': {
        id: 'ACH_CAGE_CAT', title: '笼中困兽', description: '成就：试图逃离直播生涯失败，被关进了镀金的笼子。',
        icon: Lock, color: 'text-stone-700', isGood: false, image: getImg('金丝笼里的猫', '292524')
    },
    'ACH_LONE_WOLF': {
